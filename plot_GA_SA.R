...
par(mai=c(1,1,0.2,.2))
plot(xrange,yrange,xaxt="n",type="n",xlab="p",ylab="可  满  足  性  概  率",ps=16,,cex.lab=2,font.lab=1)
...
xrange = c(0,0.2)
mars=c(5,6,4,2)+0.1
plot(xrange,yrange,xaxt='n',type="n",xlab="p",mar=mars,ylab="可  解  概  率",ps=16,cex.axis=1.2,cex.lab=2,font.lab=2)
...
for (i in 1:5){
    ps <- GA_SA_res[i,]
    lines(p,ps,type='b',lwd=1.2,lty=linetype[i],pch=plotchar[i],cex=1.5)
}

title("算法2数值实验结果",cex.main=1.5)
legend("bottomleft",inset=0.01,c('N=20','N=40','N=60','N=80','N=100'),cex=1.2,pch=plotchar,lty=linetype)
grid(nx=NA,ny=2,lwd=2)}